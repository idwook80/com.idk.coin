...
public class CalculatePositionV2 {
	public static Logger LOG 			= LoggerFactory.getLogger(CalculatePositionV2.class.getName());
	Position buy,sell;
	Balance balance;
	double price;
	double qty;
	...
	public CalculatePositionV2(BybitAlarmsModel parent,double price,Position buy, Position sell, Balance balance,double qty, boolean debug) {
		this.parent = parent;
		this.price  = price;
		this.buy 	= buy;
		this.sell	= sell;
	...
	public void calculateShortStatus(double short_price,double short_size, double current_price, int vs_size) throws Exception{
		...
		calculateShortOpenStatus(default_price, start_open, enable_open_size, is_high, price_between_size, vs_size);
		calculateShortCloseStatus(default_price, start_close, enable_close_size, is_high, price_between_size, vs_size);
	}
	
	
	
	public void calculateShortOpenStatus(double default_price, double start_open, 
		...
		if(enable_open_size < 0) {  //더이상 open 불가 short stoploss 개념
			...
			if(is_high) {
				logInfo("#   [SHORT HIGH] 현재가가 SHORT(매도가)보다 낮음(순익)		공매수(LONG) 많을가능성 많음 \t#" + between_size);
	...
	public void calculateShortCloseStatus(double default_price, double start_close, 
	...
	public void calculateLongStatus(double long_price,double long_size, double current_price,int vs_size) throws Exception{
		...
		calculateLongOpenStatus(default_price, start_open, enable_open_size, is_high, price_between_size,vs_size);
		calculateLongCloseStatus(default_price, start_close, enable_close_size, is_high, price_between_size,vs_size);
	}
	public void calculateLongOpenStatus(double default_price, double start_open, 
				int enable_open_size,boolean is_high, int price_between_size, int vs_size) throws Exception{
		logInfo("");
		logInfo("############################# LONG OPEN(BUY) STRATEGY ################################"); 
		double end_open 		= 0.0;
		int between_size		= price_between_size;
		//상승대비 OPEN은 가능 CloseAndOpen후 LongClose이  Over Open함
		//가격 하락시 전략
		if(enable_open_size < 0) {  //long stop loss 개념
			...
			//오픈 가능한 경우
			if(is_high) {
				logInfo("#   [LONG HIHG] 현재가가 더 낮음(손실)		Long 많을가능성 높음			#" );
				...
				if(between_size > 0) {
					//오픈가가 기존 오픈가보다 높아  OPEN 단가 상승함 (수익을 최대화해야함)
	...
	public void calculateLongCloseStatus(double default_price, double start_close, 
						int enable_close_size,boolean is_high,int price_between_size,int vs_size) throws Exception{
		logInfo("");
		...
		if(is_high) {
			...
			if(vs_size > 0 ) between_size += vs_size/2; 
			if(between_size > enable_close_size) between_size = enable_close_size;
			
			if(between_size > 0) {
				end_close = start_close + (between_size * INTERVAL_100);
				startOverCloseLong(start_close , end_close, INTERVAL_100, PROFIT_100, "41.[상승][클로즈 가능][단가 높음] - 단가까지 I100,P100 ("+between_size+")");
				start_close  = end_close;
				enable_close_size -= between_size;
			}
			...
			if(vs_size < 0) { //단가는 낮아 수익이 있지만 Short 사이즈가 많은 경우 상승 오픈으로 갯수if(vs_size < 0 && enable_close_size < 0) { 
				...
				int size = Math.abs(vs_size/3);
				end_close	= start_close + (size * INTERVAL_100);
				if(is_short) startOverCloseShort(start_close, end_close, INTERVAL_100, PROFIT_200,"42.[상승][반대가능][단가?] < Short 2배이상  I100,P200("+size+")");
				else startOverOpenLong(start_close, end_close, INTERVAL_100, PROFIT_200,"42.[상승][오픈가능][단가?] < Short 2배이하 I100,P200("+size+")");
				start_close = end_close + PROFIT_200;
			} 
